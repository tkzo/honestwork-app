...
import { n as networkSigner, d as userAddress, u as userConnected, e as userState } from "../../../../chunks/Network.js";
...
${!$userConnected ? `<section class="${"svelte-xva5n6"}"><div class="${"gm svelte-xva5n6"}"><div class="${"gm-inner svelte-xva5n6"}"><img src="${"icons/heart.svg"}" alt="${"Heart"}">
		...
		<div class="${"gm svelte-xva5n6"}"><p class="${"light-60"}">to be able to create a profile or create job listings; you need to connect your wallet.
			</p></div>
		<div class="${"gm link svelte-xva5n6"}"><p class="${"yellow"}">connect wallet</p></div>
		<div class="${"gm svelte-xva5n6"}"><p>skip for now</p></div></section>` : `${userState == 1 ? `<section class="${"svelte-xva5n6"}"><div class="${"gm svelte-xva5n6"}"><div class="${"gm-inner svelte-xva5n6"}"><img src="${"icons/heart.svg"}" alt="${"Heart"}">
				<div style="${"width:8px"}"></div>
				<p>gm fren (<span class="${"yellow"}">2/2</span>)</p></div></div>
		<div class="${"gm svelte-xva5n6"}"><p class="${"light-40"}">just so we can call you by your preferred name;</p>
			<div style="${"height:8px"}"></div>
			<input type="${"text"}" placeholder="${"<enter name>(must)"}" class="${"svelte-xva5n6"}"${add_attribute("value", name, 0)}>
			<div style="${"height:16px"}"></div>
			<p class="${"light-40"}">or if you want to receive email notifications;</p>
			<div style="${"height:8px"}"></div>
			<input class="${"passive-input svelte-xva5n6"}" type="${"text"}" placeholder="${"<enter email>(optional)"}"${add_attribute("value", email, 0)}></div>
		<button action="${"submit"}" class="${"svelte-xva5n6"}"><div class="${"gm svelte-xva5n6"}"><p class="${"yellow"}">create account</p></div></button>
